...
export default function WithdrawPage() {
  ...
  const handleWithdraw = () => {
    if (selectedNetwork === 'worldchain') {
      toast('Coming Soon: Worldchain withdrawals will be supported in the future', {
      ...
      <div className="flex items-center mb-6">
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Ended on {new Date(predictionResult.endDate).toLocaleDateString('en-US')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2 mb-6">
              <div className={`p-2 rounded-full ${predictionResult.result ? 'bg-green-100' : 'bg-red-100'}`}>
            ...
            <div className="space-y-4 mb-6">
              <div className="p-4 rounded-lg bg-muted">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{predictionResult.yourBet.toUpperCase()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Bet Amount</p>
                    <p className="font-medium">{predictionResult.betAmount} WLD</p>
                  </div>
                  {/* <div>
              ...
              <div className="p-4 rounded-lg bg-muted">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span>Result</span>
                    <span className={predictionResult.result === (predictionResult.yourBet === 'yes') ? 'text-green-500' : 'text-red-500'}>
                      {predictionResult.result === (predictionResult.yourBet === 'yes') ? 'Won' : 'Lost'}
                    </span>
                  </div>
                  <div className="flex justify-between font-medium">
                    <span>Available to Withdraw</span>
                    <span className={predictionResult.result === (predictionResult.yourBet === 'yes') ? 'text-green-500' : 'text-red-500'}>
                      {predictionResult.result === (predictionResult.yourBet === 'yes') 
                  ...
                  {predictionResult.result === (predictionResult.yourBet === 'yes') && (
                    <div className="mt-6">
                      <h4 className="font-medium mb-3">Select Network for Withdrawal</h4>
                      <RadioGroup
                      ...
                      >
                        <div className="flex items-center space-x-3 p-5 border rounded-lg hover:bg-muted cursor-pointer">
                          <RadioGroupItem value="optimism" id="optimism" className="h-5 w-5" />
                          <div className="flex items-center space-x-3">
                            ...
                            />
                            <Label htmlFor="optimism" className="text-lg cursor-pointer">Optimism</Label>
                          </div>
                        </div>
                        <div className="flex items-center space-x-3 p-5 border rounded-lg hover:bg-muted cursor-pointer">
                          <RadioGroupItem value="worldchain" id="worldchain" className="h-5 w-5" />
                          <div className="flex items-center space-x-3">
                            <Image
                              src="/coin.png"
                              alt="Worldcoin Logo"
                              ...
                              className="rounded-full"
                            />
                            <Label htmlFor="worldchain" className="text-lg cursor-pointer">Worldchain</Label>
                          </div>
                        </div>
                      </RadioGroup>
                    </div>
            ...
            <div className=" gap-4">
              <Button 
                ...
                disabled={predictionResult.result !== (predictionResult.yourBet === 'yes')}
              >
                {predictionResult.result === (predictionResult.yourBet === 'yes')
